...
async function main() {
  ...
  if (!hasPhotoURL) {
    ...
    await pool.query(
      "ALTER TABLE Enseignant ADD COLUMN photoURL VARCHAR(512) DEFAULT NULL COMMENT 'URL de la photo de profil'"
    );
...
main()
  ...
  .catch((err) => {
    console.error('❌ Migration photoURL (Enseignant) échouée:', err.message);
    process.exitCode = 1;
  })
  .finally(async () => {
    try {
