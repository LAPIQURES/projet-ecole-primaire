...
import { MessageSquare, Send, Search, Mail, CheckCheck, Users, GraduationCap, X } from 'lucide-react';
...
export default function TeacherMessages() {
  ...
  useEffect(() => {
    try {
      ...
      socket.on('group:created', () => { loadAll().catch(() => {}); });
  ...
  const loadAll = async () => {
    ...
    setError('');
    try {
      const [contactsRes, messagesRes, groupsRes] = await Promise.all([
        getMessageContactsAPI(), getMessagesAPI(), getGroupsAPI()
      ]);
      ...
      const groups = Array.isArray(groupsRes.data)
        ? groupsRes.data.map(g => ({ id: `group-${g.id}`, identifier: `group-${g.id}`, role: 'group', label: g.name, displayLabel: g.name }))
        : [];
      ...
      try {
        ...
        if (socket && Array.isArray(groupsRes.data)) {
          groupsRes.data.forEach((g) => { socket.emit('join:group', g.id); });
        }
      } catch (e) {}
    } catch (err) {
  ...
  }, [messages, groupMessages, selectedContact]);
  ...
  }, [messages, selectedContact, me, groupMessages]);
  ...
  const handleSend = async (e) => {
    e.preventDefault();
    if (!selectedContact) { setError('Sélectionnez un destinataire'); return; }
  ...
  return (
    <TeacherLayout title="Messages" subtitle="Communication avec l'école">
          ...
          <div style={{ padding: 16, borderBottom: '1px solid #eef2f7' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              ...
              <div style={{ marginLeft: 'auto' }}>
                <button onClick={() => setShowCreateGroup(true)} style={{ background: '#e6f0ff', border: '1px solid #dbeafe', color: '#2563eb', padding: '8px 10px', borderRadius: 10, cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>Créer un groupe</button>
              </div>
            </div>
            <div style={{ position: 'relative' }}>
              ...
              <input
                ...
                style={{ width: '100%', padding: '10px 12px 10px 36px', borderRadius: 12, border: '1px solid #dbe3ee', outline: 'none', fontSize: 13 }}
              />
            </div>
          </div>
            ...
            <div style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 12, paddingRight: 6 }}>
              {threadMessages.length === 0 ? (
                ...
                threadMessages.map((message, idx) => {
                  ...
                  return (
                    <div key={message.idMessage || message.id || idx} style={{ display: 'flex', justifyContent: mine ? 'flex-end' : 'flex-start' }}>
            ...
            <form onSubmit={handleSend} style={{ marginTop: 18, borderTop: '1px solid #eef2f7', paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 12 }}>
                <input
                  ...
                  placeholder="Objet"
                  style={{ padding: '11px 12px', borderRadius: 12, border: '1px solid #dbe3ee', outline: 'none', fontSize: 13 }}
                />
                <input
                  value={content}
      ...
      {showCreateGroup && (
        <div onClick={() => setShowCreateGroup(false)} style={{ position: 'fixed', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,6,23,0.5)', padding: 16, zIndex: 200 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 520, background: '#fff', borderRadius: 16, padding: 20, boxShadow: '0 8px 24px rgba(2,6,23,0.3)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginBottom: 8 }}>
              ...
              <button type="button" onClick={() => setShowCreateGroup(false)} style={{ padding: '8px 10px', borderRadius: 10, border: 'none', background: '#f1f5f9', color: '#475569', cursor: 'pointer' }}><X size={16} /></button>
            </div>
            <form onSubmit={handleCreateGroup}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
                  <button type="button" onClick={() => setShowCreateGroup(false)} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid #e6edf3', background: '#fff', cursor: 'pointer' }}>Annuler</button>
                  <button type="submit" style={{ padding: '8px 12px', borderRadius: 8, border: 'none', background: '#2563eb', color: '#fff', cursor: 'pointer' }}>Créer</button>
                </div>
              </div>
            </form>
          </div>
